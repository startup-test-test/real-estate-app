...
| No | 課題内容 | 原因 | 対応状況 |
|----|---------|------|----------|
| 1 | マンションが入っている | 土地(propertyType='01')で検索しているのに、マンション物件が結果に含まれている | 🟢 対応済 |
| 2 | ML分析の詳細が表示されない | 延床面積の計算が土地に対応していないため | 🟢 対応済 |
| 3 | 類似物件の取引事例が表示されない | フィルタリング条件が建物用のため、土地の場合にマッチしない | 🟢 対応済 |
