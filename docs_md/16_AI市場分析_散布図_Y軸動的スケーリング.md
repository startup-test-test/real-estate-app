...
## 最終更新
2025年9月24日 - ヒートマップへの動的スケーリング適用完了
...
2. **「2. 延床面積別価格分布」ヒートマップ**
   - 延床面積（土地の場合は土地面積）と価格の分布を2次元ヒートマップで表示
   - IQR法とハードリミットで外れ値を除去
   - 動的な価格ビンを生成（3パターン）
   - パターン1: 0-10,000万円を1,000万円刻み（11ビン）
   - パターン2: 0-20,000万円を2,000万円刻み（11ビン）
   - パターン3: 0-30,000万円を3,000万円刻み（11ビン）
   - 最終ビンは最大値を明示（10,000万円、20,000万円、30,000万円）
   - データ収集時にもIQRフィルタを適用して外れ値を除外
