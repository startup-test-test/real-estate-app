...
| テストID | シナリオ | 実施日 | 実施者 | 結果 | 不具合ID | 備考 |
|----------|---------|--------|--------|------|----------|------|
| ERROR_001 | ゼロ値入力パターン | 2025-08-09 | システム | ✅ 合格（修正済） | ~~BUG_008~~ | **【再テスト結果】物件価格0円、月額賃料0円で計算実行→「0より大きい値を入力してください」エラー表示。計算ブロック成功。NaN/Infinity防止完了** |
...
| BOUNDARY_002 | 表面利回りの境界値テスト | 2025-08-09 | システム | ✅ 合格（修正済） | ~~BUG_013~~ | **【再テスト結果】物件価格1000万円、月額賃料825万円（表面利回り99%相当）→計算実行→正常。月額賃料830万円（99.6%相当）→エラー表示「表面利回りが99.6%になります。99%を超える値は入力できません」。適切に制限** |
