...
**作成日**: 2025-08-10  
**最終更新**: 2025-08-10  
**バージョン**: 3.1.0（売却時点定義・繰越欠損FIFO追加・完全版）  
**対象システム**: 物件シミュレーター  
**緊急度**: 🔴 **最優先（有料版リリース前に必須）**
...
- **投資効率（CCR）が大幅に誤表示**される深刻な状況（ケースにより一桁〜二桁の誤差）
...
|--------|------|------------|----------|----------------|---------|
| BUG_001 | **自己資金計算式** | 改装費のみ（100万円）を使用 | `購入-借入+諸経費+改装費` | 600万円（1350-1350+500+100） | 🔴致命的 |
| BUG_002 | **CCR（初年度）** | -78.7%（分母100万円） | `年次CF÷自己資金×100` | -13.1%（-78.7÷600） | 🔴致命的 |
| BUG_003 | **CCR（全期間）** | 44.74%（売却益含む？） | `運営累計CF÷自己資金÷保有年数×100`<br>※売却CF除外 | 約15.4%（運営のみ） | 🔴致命的 |
| BUG_004 | **自己資金≤0処理** | 0.00%表示（天沼例） | `if(自己資金≤0) return "N/A"` | N/A表示 | 🟡重大 |

### 2. 改装費処理バグ群

| バグID | 項目 | 現状の問題 | 修正方法 | 期待値 | 影響度 |
|--------|------|------------|----------|---------|---------|
| BUG_005 | **改装費のCF無視** | 初年度CFに計上なし | `yearCF[0] -= renovationCost` | 初年度CF-370万円（川越例） | 🔴致命的 |
| BUG_006 | **改装費の減価償却無視** | 建物80万円/年のみ | `(建物価格+改装費)÷償却年数` | 154万円/年（400+370）÷5 | 🔴致命的 |

### 3. NOI・DSCR計算バグ群

| バグID | 項目 | 現状の問題 | 修正方法 | 期待値 | 影響度 |
|--------|------|------------|----------|---------|---------|
| BUG_007 | **NOI計算** | 固定資産税未算入<br>空室率未反映 | `(賃料×12×(1-空室率))-(管理費×12)-(その他×12)-固定資産税` | 木崎:166万円→正確な値 | 🔴致命的 |
| BUG_007b | **月額×12忘れ** | 管理費を12倍していない | 月額入力は必ず×12 | 管理費7500円→9万円/年 | 🔴致命的 |
...
| バグID | 項目 | 現状の問題 | 修正方法 | 期待値 | 影響度 |
|--------|------|------------|----------|---------|---------|
| BUG_014 | **支払利息未算入** | 税務所得に利息なし | `賃料-経費-利息-減価償却` | 税額減少 | 🔴致命的 |
| BUG_015 | **繰越欠損金無期限** | 期限設定なし | `個人:3年、法人:10年`<br>`FIFO方式（古い年度から消化）`<br>`期限切れは自動失効` | 期限切れ後は課税 | 🟡重大 |
| BUG_016 | **売却時課税誤り** | 一律譲渡所得税 | `if(個人){短期40%/長期20%}else{実効税率}` | 法人は実効税率適用 | 🟡重大 |
...
| バグID | 項目 | 現状の問題 | 修正方法 | 期待値 | 影響度 |
|--------|------|------------|----------|---------|---------|
| BUG_018 | **LTV計算基準** | 積算評価を分母？ | `借入額÷購入価格×100` | 天沼:107.5%（購入基準） | 🟡重大 |
...
|----------|----------|-----------|-----------|-----|------|----------|
| 木崎 | 600万円 | -13.1% | 約15.4% | 166万円 | 1.50 | ⬜ 未検証 |
| クレメント川越 | 410万円 | -6.4% | 要計算 | 約50万円 | 0.62 | ⬜ 未検証 |
| 北越谷 | 880万円 | -1.4% | 要計算 | 約45万円 | <0.80 | ⬜ 未検証 |
| 天沼町 | -50万円 | N/A | N/A | 270万円 | 1.08 | ⬜ 未検証 |
...
gantt
    ...
    section Phase 1（致命的）
    自己資金計算統一    :a1, 2025-08-11, 2h
    CCR計算修正         :a2, after a1, 2h
    改装費二面処理      :a3, after a2, 2h
    NOI/空室率修正      :a4, after a3, 2h
    section Phase 2（税務）
    税務所得修正        :b1, 2025-08-12, 2h
    繰越欠損/売却課税   :b2, after b1, 2h
    section Phase 3（品質）
    LTV/エラー処理     :c1, after b2, 2h
    統合テスト         :c2, after c1, 2h
```
...
### 最低限必要な確認（Phase 1完了後）
- [ ] 自己資金 = 購入-借入+諸経費+改装費で統一
- [ ] CCR（初年度）の誤差が1%以内
- [ ] CCR（全期間）が運営CFのみ（売却除外）
- [ ] 自己資金≤0でN/A表示
- [ ] 改装費がCFと減価償却の両方に反映
- [ ] NOIに空室率と固定資産税が反映
- [ ] 月額入力は全て×12されている
