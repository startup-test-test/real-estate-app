...
| BUG_013 | 表面利回り上限チェック | 【入力】表面利回り999%を入力 | 受付され月額賃料8.3億円に | エラーまたは上限99% | `/bolt_front/src/pages/Simulator.tsx` L472-504<br>99%上限チェック追加 | 中(2h) | ✅ 修正完了 | 賃料自動計算 |
...
| 不具合ID | 概要 | テスト内容 | 実際の結果 | 期待結果 | 修正内容 | 修正工数 | ステータス | 影響範囲 |
|----------|------|----------|------------|----------|----------|----------|------------|----------|
| BUG_005 | 管理手数料％入力 | 【入力】管理手数料欄に「3%」「5%」と入力 | エラーで入力不可。％記号が受け付けられない | 「3%」→自動で賃料×3%計算 | ％入力パーサーと自動計算機能追加 | 大(4h) | 🔴 未着手 | 管理費計算 |
| BUG_009 | 上限値チェック全般 | 【入力】物件価格999999999万円、金利99%等 | 全て受付。異常な計算結果表示 | 各項目に妥当な上限値設定 | 全入力フィールドに上限値バリデーション追加 | 中(3h) | 🔴 未着手 | 全計算処理 |
| BUG_010 | 必須項目チェック | 【操作】物件名・所在地を空欄で計算ボタン押下 | エラー表示なく計算実行される | 必須項目エラー表示 | 必須フィールドの空チェック追加 | 中(2h) | 🔴 未着手 | 入力検証 |
| BUG_019 | モバイル表示崩れ | 【環境】iPhone(375px幅)で表示確認 | 入力欄が画面外、グラフ横スクロール必要 | レスポンシブ対応で全要素が画面内表示 | メディアクエリとFlexbox調整 | 大(4h) | 🔴 未着手 | モバイルUI |
| BUG_021 | APIタイムアウト | 【操作】APIを故意に遅延させて計算実行 | 無限待機、画面フリーズ状態 | 30秒でタイムアウトエラー表示 | AbortControllerでタイムアウト実装 | 中(2h) | 🔴 未着手 | API通信全般 |
...
| 不具合ID | 概要 | テスト内容 | 実際の結果 | 期待結果 | 修正内容 | 修正工数 | ステータス | 影響範囲 |
|----------|------|----------|------------|----------|----------|----------|------------|----------|
| BUG_011 | 全角数字変換 | 【入力】「１２３４５」（全角）をペースト | そのまま残りNaN表示 | 自動で「12345」に変換 | 全角→半角自動変換処理追加 | 小(1h) | 🟡 未着手 | 数値入力全般 |
| BUG_012 | 未来年チェック | 【入力】建築年2026年、2050年を入力 | 受付され築年数が「-1年」等と表示 | エラー「未来の年は入力できません」 | 現在年以降の入力制限 | 小(1h) | 🟡 未着手 | 建築年入力 |
| BUG_014 | ローン期間上限 | 【入力】借入期間100年、200年を入力 | 全て受付、異常な返済額計算 | 最大50年に制限 | 借入期間の上限値チェック | 小(1h) | 🟡 未着手 | ローン計算 |
| BUG_015 | 自動計算タイミング | 【操作】物件価格変更後の管理費自動更新 | 手動入力値も上書きされる | 手動変更後は自動計算OFF | 手動変更フラグ管理 | 中(3h) | 🟡 未着手 | 自動計算全般 |
| BUG_016 | カンマ付き数値処理 | 【入力】「1,234,567」をコピペ | 「1」だけ入力される | 「1234567」として処理 | カンマ除去処理追加 | 中(2h) | 🟡 未着手 | 数値入力全般 |
| BUG_020 | 文字数上限 | 【入力】物件名に1万文字入力 | 全て受付、DB保存される | 200文字で制限 | maxLength属性追加 | 小(1h) | 🟡 未着手 | テキスト入力 |
