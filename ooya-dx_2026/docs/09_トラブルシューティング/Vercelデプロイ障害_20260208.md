...
### ビルドキャッシュの問題（追加調査 16:02頃）

復旧後もGitプッシュによるビルドがハングし続けた。パターンを分析した結果、**ビルドキャッシュが原因**の可能性が高い：

| ビルド | キャッシュ | 結果 |
|---|---|---|
| 15:49 ダッシュボードRedeploy | **なし（Skipping）** | **成功（2m 50s）** |
| 15:48 Git push | あり（Restored） | **ハング** |
| 16:02 Git push | あり（Restored） | **ハング** |

- Gitプッシュによるビルドは自動的にキャッシュを使用する
- **キャッシュなし → 成功、キャッシュあり → ハング**の傾向
- ビルドキャッシュが破損している可能性

#### 対処法

1. **Settings → Caches → Purge Everything** でビルドキャッシュを全削除
2. 次の成功ビルドで新しいキャッシュが自動生成される
3. 新キャッシュで正常にビルドできれば、以降はキャッシュありでもOK
...
月10回デプロイでも**+8分程度**のため、コストへの影響は軽微。

### 復旧確認済み

- [x] Gitトリガービルドがキャッシュなしで正常に動作することを確認
- [x] リッチリザルトテストで構造化データの有効性を確認
- [ ] **ビルドキャッシュをPurgeしてGitプッシュで正常にビルドされるか確認**
- [ ] Vercelサポートからの回答を待つ
