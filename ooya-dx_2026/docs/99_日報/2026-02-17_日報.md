...
### 3. template-demo を ToolPageLayout ベースに書き直し

`/tools/template-demo` が古い実装（Header/Footer直接配置、1カラム）のままだったため、実際のツールページと同じ `ToolPageLayout`（2カラムLP型）ベースに書き直し。

| | 変更前 | 変更後 |
|---|--------|--------|
| テンプレート | Header/Footer直接配置 | **ToolPageLayout** |
...
ToolPageLayoutとの比較用に、layout.tsxだけのサンプルページを作成。

| テンプレート | サンプルURL | 構造 |
|-------------|-----------|------|
| layout.tsx（公開ページ共通） | `/layout-sample` | シンプル1カラム |
| ToolPageLayout | `/tools/template-demo` | 2カラムLP型 |
...
| tools のlayout.tsx化 | する or しない | **しない** | ToolPageLayoutがLP型で十分機能しており、layout.tsx化のメリットが薄い |
| pricing/faqの統一 | する or しない | **する** | 他の公開ページと見た目を揃えるため |
| 横スクロール対策 | 個別修正のみ or layout一括 | **両方** | 根本原因（画像）修正 + layout.tsxに安全策追加 |
| template-demo | 削除 or 書き直し | **書き直し** | ToolPageLayoutベースのサンプルとして活用 |
