...
// シミュレーター本体
function NPVSimulator() {
  // 入力状態
  const [initialInvestmentInMan, setInitialInvestmentInMan] = useState<number>(0)
  const [annualCashFlowInMan, setAnnualCashFlowInMan] = useState<number>(0)
  const [holdingPeriodYears, setHoldingPeriodYears] = useState<number>(10)
  const [discountRate, setDiscountRate] = useState<number>(5)
  const [terminalValueInMan, setTerminalValueInMan] = useState<number>(0)
  ...
  return (
    <>
        ...
        {result && (
          <div className="bg-white rounded-lg p-3 sm:p-4">
            ...
            <div className="grid grid-cols-2 gap-y-3 text-sm sm:text-base">
              ...
              <span className={`text-right text-xl sm:text-2xl font-bold border-t-2 border-blue-300 pt-4 mt-2 ${result.npv >= 0 ? 'text-blue-700' : 'text-red-600'}`}>
      ...
      {hasInput && sensitivityAnalysis.length > 0 && (
        <section className="mt-8 mb-8 sm:mb-12">
          <h2 className="text-base sm:text-lg font-bold text-gray-900 mb-4">割引率別NPV感度分析</h2>
            ...
            <table className="w-full text-xs sm:text-sm">
              <thead>
                <tr className="bg-gray-100">
                  <th className="px-2 sm:px-3 py-2 text-left font-medium text-gray-700">割引率</th>
                  <th className="px-2 sm:px-3 py-2 text-right font-medium text-gray-700">NPV</th>
                  <th className="px-2 sm:px-3 py-2 text-right font-medium text-gray-700">PI</th>
                </tr>
              </thead>
              <tbody>
                {sensitivityAnalysis.map((row) => (
                  ...
                  >
                    <td className={`px-2 sm:px-3 py-2 ${row.isSelected ? 'font-bold text-blue-700' : ''}`}>
                      {row.rate}%{row.isSelected && ' ← 選択中'}
                    </td>
                    <td className={`px-2 sm:px-3 py-2 text-right ${row.npv >= 0 ? 'text-green-600' : 'text-red-600'} ${row.isSelected ? 'font-bold' : ''}`}>
                      {row.npv >= 0 ? '+' : ''}{Math.round(row.npv).toLocaleString()}万円
                    </td>
                    <td className={`px-2 sm:px-3 py-2 text-right ${row.pi >= 1 ? 'text-green-600' : 'text-red-600'} ${row.isSelected ? 'font-bold' : ''}`}>
                      {row.pi.toFixed(2)}
                    </td>
                  </tr>
                ))}
...
// 追加コンテンツ
function NPVAdditionalContent({ relatedGlossary }: { relatedGlossary: GlossaryItem[] }) {
  return (
    <>
      {/* 目次 */}
...
export function NPVCalculator({ relatedGlossary = [] }: NPVCalculatorProps) {
  return (
    <ToolPageLayout
      title={PAGE_TITLE}
      toolPath="/tools/npv"
      additionalContent={<NPVAdditionalContent relatedGlossary={relatedGlossary} />}
    >
      <NPVSimulator />
    </ToolPageLayout>
  )
