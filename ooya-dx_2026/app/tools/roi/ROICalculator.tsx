...
export function ROICalculator({ relatedGlossary = [] }: ROICalculatorProps) {
  return (
    <ToolPageLayout
      title={PAGE_TITLE}
      toolPath="/tools/roi"
      additionalContent={<ROIAdditionalContent relatedGlossary={relatedGlossary} />}
    >
      <ROISimulator />
    </ToolPageLayout>
  )
...
function ROISimulator() {
  ...
  const [expenseRatePercent, setExpenseRatePercent] = useState<number>(20)
  const [loanAmountInMan, setLoanAmountInMan] = useState<number>(0)
  const [annualLoanPaymentInMan, setAnnualLoanPaymentInMan] = useState<number>(0)
  ...
  const result = useMemo(() => {
    if (propertyPriceInMan <= 0) return null
    return calculateROI({
      propertyPriceInMan,
      purchaseCostsInMan,
      annualRentInMan,
      ...
      {result && (
        <div className="bg-white rounded-lg p-3 sm:p-4">
          <div className="grid grid-cols-2 gap-y-3 text-sm sm:text-base">
            ...
            <span className={`text-right text-xl sm:text-2xl font-bold border-t-2 border-blue-300 pt-4 mt-2 ${result.cfRoi >= 0 ? 'text-blue-700' : 'text-red-600'}`}>
 ...
 * ROIページ固有の追加コンテンツ
 */
function ROIAdditionalContent({ relatedGlossary }: { relatedGlossary: GlossaryItem[] }) {
  return (
    <>
      {/* 目次 */}
