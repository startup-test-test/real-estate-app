...
import { validatePropertyUrl } from '@/lib/utils/validation';
import { validateSimulatorInputs } from '@/lib/utils/securityValidation';
import { transformFormDataToApiData } from '@/lib/utils/dataTransform';
import { emptyPropertyData } from '@/lib/constants/sampleData';
import { tooltips } from '@/lib/constants/tooltips';
import { sampleProperty } from '@/data/sampleProperty';
import { propertyStatusOptions, loanTypeOptions, ownershipTypeOptions, buildingStructureOptions, corporateTaxRateOptions, individualTaxRateOptions } from '@/lib/constants/masterData';
          ...
          <div className="border-t pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (newOwnershipType === '個人') {
                      handleInputChange('effectiveTaxRate', 20); // 個人のデフォルト（課税所得695万円以下）
                    } else if (newOwnershipType === '法人') {
                      handleInputChange('effectiveTaxRate', 30); // 法人のデフォルト（一般法人）
                    }
                  }}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                >
              ...
              <div>
                <div className="flex items-center mb-2">
                  <label className="text-sm font-medium text-gray-700">
                    {inputs.ownershipType === '法人' ? '実効税率' : '所得税率'}
                  </label>
                ...
                <div className="space-y-2">
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  >
                    {(inputs.ownershipType === '法人' ? corporateTaxRateOptions : individualTaxRateOptions).map(option => (
                      <option key={option.value} value={option.value}>
                        {option.label}
                      </option>
                    ))}
