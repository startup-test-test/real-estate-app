...
export const transformFormDataToApiData = (inputs: any) => {
  ...
  const result = {
    ...
    loan_years: inputs.loanYears !== undefined ? inputs.loanYears : 35,
    loan_type: inputs.loanType || '元利均等',
    holding_years: 35,  // 常に35年分のデータを要求
    exit_cap_rate: Number(inputs.exitCapRate) || 0,
    expected_sale_price: Number(inputs.marketValue) || 0,
    price_decline_rate: inputs.priceDeclineRate || 0,
    ownership_type: inputs.ownershipType || '個人',
    effective_tax_rate: inputs.effectiveTaxRate !== undefined ? inputs.effectiveTaxRate : 20,
    major_repair_cycle: inputs.majorRepairCycle !== undefined ? inputs.majorRepairCycle : 0,
    major_repair_cost: inputs.majorRepairCost !== undefined ? inputs.majorRepairCost : 0,
    building_price: inputs.buildingPriceForDepreciation || (inputs.purchasePrice || 0) * 0.7,
    depreciation_years: inputs.depreciationYears !== undefined ? inputs.depreciationYears : 27,
...
export const transformApiResponseToSupabaseData = (inputs: any, result: any, shareToken?: string) => {
  const simulationData = {
    ...
    priceDeclineRate: inputs.priceDeclineRate || 0,
  ...
  const resultData = {
    ...
    '純利益': result['純利益'] || 0,
    'ROI': result['ROI'] || 0,
    'ROI（初年度）（%）': result['ROI（初年度）（%）'] || 0,
    'ROI（全期間）（%）': result['ROI（全期間）（%）'] || 0,
    'IRR': result['IRR'] || 0,
    'NPV': result['NPV'] || 0,
    '投資効率': result['投資効率'] || 0,
    'CCR': result['CCR'] || 0,
    'CCR（初年度）（%）': result['CCR（初年度）（%）'] || 0,
    'CCR（全期間）（%）': result['CCR（全期間）（%）'] || 0,
    'DSCR': result['DSCR'] || 0,
    'LTV': result['LTV'] || 0,
    '実質利回り': result['実質利回り'] || 0,
    '表面利回り': result['表面利回り'] || 0,
    'CF累計': result['CF累計'] || 0,
...
export const transformSupabaseDataToFormData = (simulationData: any) => {
  return {
    ...
    priceDeclineRate: simulationData.priceDeclineRate ?? 0,
