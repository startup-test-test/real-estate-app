...
class SessionSecurityManager:
    ...
    def create_session(self, user_id: str, request: Request) -> Tuple[str, SessionData]:
        ...
        if len(user_sessions) >= MAX_SESSIONS_PER_USER:
            ...
            logger.warning(f"Max sessions reached for user {user_id}, removing oldest session")
        ...
        logger.info(f"New session created for user {user_id}: {session_id[:8]}...")
    ...
    def validate_session(self, session_id: str, request: Request) -> Optional[SessionData]:
        ...
        if session.fingerprint != current_fingerprint:
            logger.warning(
                f"Session fingerprint mismatch for {session_id[:8]}... "
        ...
        if session.ip_address != current_ip:
            logger.warning(
                f"IP address changed for session {session_id[:8]}... "
                f"from {session.ip_address} to {current_ip}"
            )
    ...
    def rotate_session(self, old_session_id: str, request: Request) -> Optional[SessionData]:
        ...
        logger.info(
            f"Session rotated for user {old_session.user_id}: "
            f"{old_session_id[:8]}... -> {new_session_id[:8]}..."
        )
    ...
    def destroy_session(self, session_id: str) -> bool:
        ...
        logger.info(f"Session destroyed: {session_id[:8]}... for user {session.user_id}")
    ...
    def record_failed_attempt(self, identifier: str) -> None:
        ...
        if self._failed_attempts[identifier] >= 5:
            logger.warning(f"Multiple failed login attempts from {identifier}")
