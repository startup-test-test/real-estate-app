...
|----|--------|-----------|----------|----------|------------|----------|
| 1 | **元金返済額** | 全年度で0円 | キャッシュフロー表 | `calculate_principal_payment()`関数を実装 | 🔴 未着手 | 2時間 |
| 2 | **借入残高** | 全年度で0円 | キャッシュフロー表、売却分析 | `calculate_remaining_loan()`を各年で呼び出し | 🔴 未着手 | 2時間 |
| 3 | **自己資金回収率** | 全年度で0% | 投資効率分析 | 累計CF ÷ 自己資金の計算を実装 | 🔴 未着手 | 1時間 |
| 4 | **売却時手取り** | 全年度で0円 | 出口戦略分析 | 売却価格 - コスト - 残債 - 税金の計算 | 🔴 未着手 | 3時間 |
| 5 | **実質利回り** | 未実装 | 投資分析 | (NOI - 税金) ÷ 購入価格 × 100 の計算を追加 | 🔴 未着手 | 1時間 |
...
| No | 項目名 | 現在の状態 | 影響範囲 | 対応方法 | ステータス | 見積時間 |
|----|--------|-----------|----------|----------|------------|----------|
| 9 | **元金均等返済** | 未対応 | ローン計算 | loan_type判定を追加 | 🟢 機能追加 | 3時間 |
| 10 | **繰上返済シミュレーション** | 未実装 | 返済計画 | 新規機能として実装 | 🟢 機能追加 | 5時間 |
| 11 | **シナリオ分析機能** | 未実装 | リスク分析 | 金利変動、賃料下落シミュレーション | 🟢 機能追加 | 8時間 |
| 12 | **融資条件の最適化** | 未実装 | 投資判断 | 複数の融資パターン比較 | 🟢 機能追加 | 6時間 |
